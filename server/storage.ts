...
class DrizzleStorage implements IStorage {
  ...
  async getRealTimeCashState(clientId: number): Promise<any> {
    try {
      ...
      const [incomeResult] = await db.execute(sqlOperator`
        SELECT 
      ...
      const aggregatedData = incomeResult as any;
  ...
  async scheduleCashOperations(clientId: number): Promise<{
    ...
    // Use configured hours consistently
    ...
    // Get Argentina timezone time consistently
    const now = new Date();
    const argentinaTime = new Date(now.toLocaleString("en-US", {timeZone: "America/Argentina/Buenos_Aires"}));

    // Schedule operations based on configuration
    ...
    // If we're past closing time today, move to tomorrow
    if (argentinaTime >= nextClose) {
      nextClose.setDate(nextClose.getDate() + 1);
    }

    // If we're past opening time today, move to tomorrow
    if (argentinaTime >= nextOpen) {
      nextOpen.setDate(nextOpen.getDate() + 1);
    }

    console.log(`📅 Schedule for client ${clientId}: Current ${argentinaTime.toLocaleString()}, Next Open: ${nextOpen.toLocaleString()}, Next Close: ${nextClose.toLocaleString()}`);

    return {
      nextClose,
      nextOpen,
      status: argentinaTime >= nextOpen && argentinaTime < nextClose ? 'open' : 'closed'
    };
  }
